...
class WebsiteDownloader:
    ...
    def process(self):
        ...
        # 10. Handle SPA frameworks (Gatsby, Next.js, Nuxt, React/Vite, etc.)
        ...
        # Detect standard React/Vite/CRA apps (id="root" + type="module" script)
        # These re-mount on #root when JS runs, wiping the captured HTML
        ...
        if is_gatsby or is_nextjs or is_nuxt or is_react_vite:
            ...
            for script in soup.find_all('script'):
                ...
                if not is_safe:
                    ...
                    # React/Vite patterns - remove all local module scripts
                    # The rendered HTML was already captured, React would clear #root on re-mount
                    if is_react_vite:
                        script_type = script.get('type', '')
                        # Remove type="module" scripts (Vite bundles)
                        if script_type == 'module' and src and not src.startswith(('http://', 'https://', '//')):
                            should_remove = True
                        # Remove modulepreload hints (rel="modulepreload")
                        # Remove React runtime data scripts
            ...
            for link in soup.find_all('link', rel=lambda r: r and any(x in r for x in ['preload', 'prefetch', 'modulepreload'])):
                ...
                # For React/Vite: remove all modulepreload links (they reference JS chunks)
                if is_react_vite:
                    rel = link.get('rel', [])
                    if isinstance(rel, list):
                        rel_str = ' '.join(rel)
